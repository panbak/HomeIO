...
const UpdateDeviceWifiCredentialsScreen = ({ route, navigation: { goBack }, navigation }) => {

    const _BleManager = new BleManager();

    const [refreshing, setRefreshing] = useState(false);

    const [buttonText, setButtonText] = useState("Αποθήκευση");
    ...
    const deviceObj = route.params.device;

    useEffect(() => {
        if (scanCompleted) {
            console.log(JSON.stringify(devices));
        }
    }, [devices, scanCompleted])
        ...
        _BleManager.startDeviceScan(null, {
            ...
            async (error, device) => {
                ...
                if (device.localName == 'HomeIO' || device.name == 'HomeIO Plug' || device.name == 'HomeIO Device') {
                    console.log("INSIDE");
                    let code = await getDeviceCode(device);
                    console.log(code);
                    if (code) {
                        _BleManager.stopDeviceScan();
                    }
    ...
    const getDeviceCode = async device => {
        console.log("CALLED");
        _BleManager.stopDeviceScan();
                    ...
                    <View style={[tailwind('mt-3 flex')]}>
                        {
                            !device ?
                                ...
                                :
                                <>
                                    <View style={[tailwind('w-full mb-2 p-4 flex rounded')]}>
                                        <View style={[tailwind('flex flex-row justify-around')]}>
                                            <Text style={[tailwind('text-gray-900 text-center text-lg font-bold')]} >{deviceObj.name}</Text>
                                        </View>
                                        <TextInput
...
export default UpdateDeviceWifiCredentialsScreen;